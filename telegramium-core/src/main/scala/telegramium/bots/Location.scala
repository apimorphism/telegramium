 ...
 * @param livePeriod Optional. Time relative to the message sending date, during
 ...
 * @param proximityAlertRadius Optional. Maximum distance for proximity alerts about
 * approaching another chat member, in meters. For sent live
 * locations only.
 */
final case class Location(longitude: Float,
                          latitude: Float,
