...
/** This object contains information about the user whose identifier was shared with the bot using a
  ...
  *   or double-precision float type are safe for storing this identifier. The bot may not have access to the user and
  *   could be unable to use this identifier, unless the user is already known to the bot by some other means.
  */
final case class UserShared(requestId: Int, userId: Long)
